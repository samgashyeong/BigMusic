...
class  TagArtistAdapter(val DataList:ArrayList<Artist>): RecyclerView.Adapter<TagArtistAdapter.MyViewHolder>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.tag_item_artist_layout, parent, false)
        return MyViewHolder(view)
    }

    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        //ex)holder.(홀더클래스변수).text = DataList[position].name
        holder.item_tag_artist.text = DataList[position].name
